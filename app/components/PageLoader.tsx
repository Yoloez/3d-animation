...
export default function PageLoader({ children, minLoadTime = 2000 }: PageLoaderProps) {
    ...
    <>
      ...
      <div className={`fixed inset-0 z-9999 flex flex-col items-center justify-center bg-primary overflow-hidden transition-all duration-600 ${isExiting ? "opacity-0 scale-110" : "opacity-100 scale-100"}`}>
        ...
        <div className="mt-12 flex flex-col items-center gap-4 z-10">
          ...
          <p className="text-secondary/60 text-sm tracking-wider">{progress < 100 ? "Loading Experience..." : "Welcome!"}</p>
        </div>
      ...
      {/* Preload the actual content behind the loader */}
      <div className={isExiting ? "opacity-100" : "opacity-0"}>{children}</div>
    </>
  );
}
